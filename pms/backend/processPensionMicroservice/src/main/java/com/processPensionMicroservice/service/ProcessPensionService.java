...
public class ProcessPensionService {
	...
	/*public double getPension(PensionerDetail pd) {
		double pension = 0;
		PensionDetail p;
		if (pd.getPensionType().equalsIgnoreCase("self"))
			pension = (pd.getSalary() * 0.8 + pd.getAllowance());
		else if (pd.getPensionType().equalsIgnoreCase("family"))
			pension = (pd.getSalary() * 0.5 + pd.getAllowance());
		//p = new PensionDetail(pd.getName(), pd.getDateOfBirth(), pd.getPan(), pd.getPensionType(), pension);
		return pension;
